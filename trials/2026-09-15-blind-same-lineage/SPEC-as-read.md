...
- **JSON-equal**: two values whose canonical forms (section 3.2) are identical. Object key order is ignored; array order is not.
...
1. Index fields by `id`. A field whose `id` is not a non-empty string, or repeats an indexed id, adds problem `missing-or-duplicate-field-id` and is not indexed.
...
4. If there is any problem: status `unknown`, `sharedSources` empty, both `independentRoots` lists empty.
...
| `missing-or-duplicate-field-id` | A field id is not a non-empty string or repeats (the repeat is not indexed). |
...
Transitive sources are computed over indexed fields, following `sources` arrays and ignoring ids that name no field.
...
2. Dispositions: the reference is `field` when it is a non-empty string, else the empty string. A reference that names no sent field: `unknown-field-disposition:ref`. A second disposition for the same field: `duplicate-disposition:ref`; the first one counts.
3. For each sent field F:
...
6. For every counted disposition `accept` or `verify` of field F: if a transitive source of F in R is marked modified, warning `ancestor-modified:F`.
...
3. A reveal item whose `field` is not a non-empty string, or repeats: `duplicate-or-invalid-reveal`; the first one counts.
...
Run 7.3 on `sent`, 7.4 on the hop and, when a reveal is given, 7.5.
...
| Kind | Cases | Of which |
|---|---|---|
| Lineage (7.1) | 18 | 8 migrated from v0.1, 4 from terminator2-agent's counterexamples (verbatim and declared) |
...
- The suite's expected outcomes, except the external inputs, were written by the same author as the reference checker. Mutation testing (14 deliberately broken checkers, all detected) reduces but does not remove this bias. Section 9 states the remedy.
...
- v0.1 convention and trial checker: `../README.md`, `../feedback-trial/`.
